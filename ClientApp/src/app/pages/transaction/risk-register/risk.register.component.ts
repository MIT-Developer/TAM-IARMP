...
export class RiskRegisterComponent {
  ...
  ngOnInit() {
    ...
    this.service.getreq("TbMComInputs").subscribe(response => {
      this.selectors.companyKpi = response.filter(object => object.flagActive == 'Active' && object.condition == 'KPI');
    });
    //get financial
  ...
  changeDivision(){
    this.selectors.department = this.data.department.filter(object=> return (object.kodeDivisi == this.state.division));
  }
  changeDepartment(){
